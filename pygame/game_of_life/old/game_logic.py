...
# keyboard input
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                ...
                state.rle1 = state.rle1.replace("\n","").replace(" ","")
                paste_rle(state.grid, state.rle1, state.cols // 2, state.rows // 2)
            elif event.key == pygame.K_2:
                state.rle2 = state.rle2.replace("\n","").replace(" ","")
                paste_rle(state.grid, state.rle2, state.cols // 2, state.rows // 2)
# mouse input
        if state.paused:
...
def paste_rle(grid, rle_string, start_x, start_y):
